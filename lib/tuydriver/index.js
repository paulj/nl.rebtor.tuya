...
module.exports = {
SearchDevices: async function (Category, Model, Productid, SubDeviceType, APIKey, APISecret, APIRegion, DeviceID) {
...
const url = 'https://openapi.tuyaeu.com';
	...
	const prettyDevices = Object.values(groupedDevices).map(device => {
        ...
        if (SubDeviceType == 'FanLight' && Category == 'fs') {
            pretty.name = pretty.name + ' Light';
            pretty.data.IP = pretty.data.IP + '-light';
        }
    ...
    processdata: function (device, data, type) { // check if data is valid
        ...
        if (typeof data == 'object') {
            ...
            if (data.dps.hasOwnProperty('1') == true && type == 'onoff') {
                console.log("setCapabilityValue", 'onoff', data.dps['1']);
            ...
            if (data.dps.hasOwnProperty('1') == true && type == 'Fan') {
                device.setCapabilityValue('onoff', data.dps['1'])
                .catch(err => {
                    console.error(err);
                });
            ...
            if (data.dps.hasOwnProperty('102') == true && type == 'FanLight') {
                device.setCapabilityValue('onoff', data.dps['102'])
                .catch(err => {
                    console.error(err);
                });
            }
            ...
            // fan
            if (data.dps.hasOwnProperty('2') == true && type == 'Fan') {
                console.log(data.dps, parseInt(data.dps['2']) / 3.0);
                device.setCapabilityValue('fan_speed', parseInt(data.dps['2']) / 3.0)
                .catch(err => {
                    console.error(err);
                });
            }
    ...
    sendvalues: function (device, APIdevice, value, parameter) {
        ...
        } else if (parameter == 'fan_speed') {
            return APIdevice.set({
                dps: 2,
                set: Math.round(value * 3).toString()
            })
            .catch(err => {
                console.error(err);
            });
        
        } else if (parameter == 'onoff.light') {
            return APIdevice.set({
                dps: 102,
                set: value
            })
